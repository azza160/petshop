...
<section id="top-animals" class="relative py-20 bg-white" x-data="topAnimalsCarousel()" x-init="initCarousel()">
        ...
        <div class="relative w-full mx-auto" @mouseenter="pause()" @mouseleave="play()">
            ...
            <div class="overflow-hidden w-full">
                ...
                <div class="flex transition-transform duration-500 ease-in-out"
                    ...
                    <!-- Loop through animals from DB -->
                    @forelse ($topAnimals as $index => $animal)
                            ...
                            <div class="bg-white border border-slate-200 shadow-sm rounded-md overflow-hidden group hover:shadow-lg transition-all duration-300 h-full flex flex-col relative"
                                ...
                                <div class="p-5 flex flex-col flex-1">
                                    ...
                                    <div class="flex items-center justify-between mb-2">
                                        <span
                                            class="text-xs font-semibold text-primary bg-primary/10 px-2 py-1 rounded-sm uppercase tracking-wider">{{ $animal->category->nama ?? '-' }}</span>
                                        <span
                                            class="text-xs text-muted font-medium bg-slate-100 px-2 py-1 rounded-sm flex items-center gap-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                            <span>{{ $animal->umur }} Bulan</span>
                                        </span>
                                    </div>

                                    <h3
                                        class="text-lg font-bold text-dark mt-1 mb-2 group-hover:text-primary transition-colors line-clamp-1">
                                        {{ $animal->nama }}</h3>
                                    <p class="text-sm text-muted line-clamp-2 mb-4 flex-1">{{ $animal->deskripsi }}</p>

                                    <div class="flex items-center justify-between pt-4 border-t border-slate-100 mb-4">
                                        <div class="flex flex-col">
                                            <span
                                                class="text-[10px] text-muted mb-0.5 uppercase tracking-wider font-semibold">Harga</span>
                                            <span class="text-lg font-bold text-secondary">Rp
                                                {{ number_format($animal->harga, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="text-right">
                                            <span
                                                class="text-xs text-emerald-600 font-medium bg-emerald-50 px-2 py-1 rounded-sm border border-emerald-100">
                                                {{ $animal->stok > 0 ? 'Tersedia' : 'Habis' }}
                                            </span>
                                        </div>
                                    </div>
                    ...
                    @empty
                        <div class="flex-none px-3 w-full text-center py-16 text-muted">
                            <i class="ph ph-paw-print text-5xl text-slate-300 mb-3 block"></i>
                            <p class="font-semibold">Belum ada hewan favorit yang ditampilkan.</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <!-- View All Button - CTA Style -->
            <div class="flex justify-center mt-12" data-aos="fade-up">
                <a href="{{ route('landing.list-hewan') }}"
                    class="flex items-center justify-center gap-3 w-full sm:w-auto px-6 py-3.5 md:px-8 bg-primary border border-primary text-white font-bold rounded-md hover:bg-emerald-600 hover:border-emerald-600 
                    transition-all duration-300 ease-in-out shadow-lg shadow-primary/30 hover:scale-105
                    group">
