<section class="py-24 bg-slate-50 relative overflow-hidden" id="testimonial"
    x-data="testimonialCarousel()" x-init="initCarousel()">
    ...
    <div class="px-4 xl:max-w-[1300px] mx-auto z-10 relative">
        ...
        <div class="text-center max-w-2xl mx-auto -mt-6 mb-12 lg:mb-16 text-muted font-medium" data-aos="fade-up" data-aos-delay="100">
            Dengarkan langsung dari para pecinta hewan bagaimana layanan kami telah membantu merawat dan memberikan kebahagiaan untuk peliharaan kesayangan mereka.
        </div>

        <!-- Carousel Wrapper with Side Buttons Layout -->
        <div class="relative flex items-center justify-center gap-4 lg:gap-8 w-full max-w-[1400px] mx-auto group/wrapper">
            ...
            <button @click="prev()" 
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 lg:w-7 lg:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <template x-for="(review, index) in testimonials" :key="index">
                        ...
                        <div class="flex-none px-4 py-2" :style="`width: ${100 / itemsPerSlide}%`">
                            ...
                            <div class="bg-white rounded-[2rem] p-8 shadow-md border border-slate-100 relative group transition-all duration-500 ease-out hover:-translate-y-3 hover:shadow-2xl hover:shadow-primary/15 h-full flex flex-col justify-between"
                                 :class="currentIndex <= index && index < (currentIndex + itemsPerSlide) ? 'opacity-100 scale-100' : 'opacity-50 scale-95 pointer-events-none'">
                                
                                <!-- Quote Icon -->
                                <div class="absolute top-6 right-8 text-6xl text-primary/10 font-serif leading-none group-hover:text-primary/20 group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500 origin-center">
                                    &rdquo;
                                </div>
                                
                                <div class="relative z-10 flex flex-col h-full">
                                    <!-- Stars -->
                                    <div class="flex gap-1 mb-6 text-secondary transform group-hover:scale-105 origin-left transition-transform duration-300">
                                    ...
                                    <!-- Content -->
                                    <p class="text-dark/80 text-lg leading-relaxed italic flex-1" x-text="review.quote"></p>
                                    
                                    <!-- User Info Profile -->
                                    <div class="mt-8 flex items-center gap-4 border-t border-slate-100 pt-6 transform group-hover:translate-x-2 transition-transform duration-500">
                                        <div class="relative overflow-hidden rounded-full w-14 h-14 border-2 p-0.5 transition-colors duration-500" :class="review.colorBorder">
                                            <img :src="review.image" :alt="review.name" class="w-full h-full rounded-full object-cover group-hover:scale-110 transition-transform duration-500">
                                        </div>
                                        <div>
                                            <h4 class="font-bold text-dark group-hover:text-primary transition-colors duration-300" x-text="review.name"></h4>
                                            <p class="text-sm font-medium" :class="review.colorText" x-text="review.role"></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="flex flex-col items-center justify-center w-full max-w-md mx-auto mt-6 px-4">
            ...
            <div class="flex md:hidden items-center justify-center gap-4 mb-6">
                <button @click="prev()" class="w-10 h-10 flex items-center justify-center rounded-full bg-white text-slate-600 border border-slate-200 shadow-sm hover:text-primary active:scale-95 transition-all focus:outline-none">
                ...
                <button @click="next()" class="w-10 h-10 flex items-center justify-center rounded-full bg-white text-slate-600 border border-slate-200 shadow-sm hover:text-primary active:scale-95 transition-all focus:outline-none">
